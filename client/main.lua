    ...
    doorMenu.Add:Button('Save & Select Door', { success = true }, function()
        ...
        if Config.Debug then
            ...
            for _, restriction in ipairs(doorConfig.restricted) do
                ...
                local workplaceDisplay = (restriction.workplace == false or restriction.workplace == '') and 'false' or restriction.workplace
                print("Job: " .. restriction.job .. ", Workplace: " .. workplaceDisplay .. ", Grade: " .. restriction.gradeLevel)
            end
        end
...
function SaveDoorConfig(door)
    ...
    for _, restriction in ipairs(door.restricted) do
        ...
        restrictionsFormatted = restrictionsFormatted .. string.format([[ 
            { type = '%s', job = '%s', workplace = %s, gradeLevel = %d, jobPermission = %s, reqDuty = %s },
        ]], restriction.type, restriction.job, workplaceValue, restriction.gradeLevel, jobPermissionValue, reqDutyValue)
    end

    -- Remove trailing comma from the restrictionsFormatted string
    restrictionsFormatted = restrictionsFormatted:sub(1, -3)
    ...
    ]], door.id, door.model, door.coords.x, door.coords.y, door.coords.z, lockedValue, door.double and '"' .. door.double .. '"' or 'nil', restrictionsFormatted)
